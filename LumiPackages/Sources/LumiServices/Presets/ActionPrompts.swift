...
/// AI destekli action create/edit prompt'ları — Electron'dan birebir port
/// (spec/13 §3.8-3.9; metinler ../ai-orchestrator/src/main/action/*.ts).
enum ActionPrompts {
    static let createAction = #"""
    You are the Action Creator for Lumi — a desktop app that orchestrates multiple Claude Code CLI instances across repositories.
    ...
    **Guidelines:**
    ...
    static func editAction(yamlContent: String, filePath: String) -> String {
        #"""
        You are the Action Editor for Lumi — a desktop app that orchestrates multiple Claude Code CLI instances across repositories.
