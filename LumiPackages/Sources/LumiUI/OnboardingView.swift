...
struct OnboardingView: View {
    ...
    private var welcomeStep: some View {
        VStack(spacing: 16) {
            ...
            Text("Birden çok Claude Code oturumunu tek panelden yönet")
            ...
            .frame(width: 240)
            .labelsHidden()
            Text("AI sağlayıcısı — sonradan Settings'ten değiştirilebilir")
    ...
    private var projectsRootStep: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Projects Root")
                .font(.system(size: 18, weight: .semibold, design: .monospaced))
                .foregroundStyle(Theme.textPrimary)
            Text("Repolarının yaşadığı klasör — birinci seviye alt dizinler listelenir")
                .font(.system(size: 11.5, design: .monospaced))
                .foregroundStyle(Theme.textSecondary)
            HStack(spacing: 8) {
                Text(projectsRoot.isEmpty ? "(seçilmedi)" : projectsRoot)
    ...
    private var readyStep: some View {
        VStack(spacing: 14) {
            ...
            Text("Hazırsın")
                .font(.system(size: 18, weight: .semibold, design: .monospaced))
                .foregroundStyle(Theme.textPrimary)
            Text("Repo aç, terminal başlat, agent'ları izle")
                .font(.system(size: 12, design: .monospaced))
                .foregroundStyle(Theme.textMuted)
        }
    }
