...
/// `GitSidebar.relativeTime` — commit timeline'ının SAF "Xm/Xh/Xd ago"
...
final class RelativeTimeTests: XCTestCase {
    ...
    private func label(secondsAgo: TimeInterval) -> String {
        GitSidebar.relativeTime(now.addingTimeInterval(-secondsAgo), now: now)
    }
    ...
    func testFutureDateClampsToZeroMinutes() {
        XCTAssertEqual(
            GitSidebar.relativeTime(now.addingTimeInterval(3600), now: now),
            "0m ago",
