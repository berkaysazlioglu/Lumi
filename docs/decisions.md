...
### 45. Plastic SCM (salt-okunur) Source Control desteği (2026-09-07)
Unity projelerinin çoğu Plastic SCM (Unity Version Control) çalışma alanıdır ve Git olmadığı için Source Control sekmesi hiç görünmüyordu. Sekme artık `.plastic/` kökü tanınan dizinlerde de açılır; kapsam bilinçli olarak **hafif** tutulur: Git paritesi (graph, diff, commit, PR) hedeflenmez.
...
- **Servis:** `PlasticReading` (LumiKit) sessiz-boş sözleşmeli, **yalnız okuma** yüzeyidir: `isCLIAvailable`, `workspaceInfo`, `status`, `recentChangesets`. Checkin/undo/update/switch **kapsam dışı** (karar 9'un push/pull dışlaması burada da geçerlidir). `PlasticService` bir `actor`dır: `cm` binary'si `BinaryLocating` ile bir kez çözülür ve saklanır (sabit path yok; `/usr/local/bin/cm` tipik). Komutlar: `cm status --header --machinereadable --fieldseparator=|` (changeset + repo@server), `cm showselector` (branch — `smartbranch "/main"`), `cm status --short --machinereadable --fieldseparator=| --all --cutignored` (değişiklikler; ignored listelenmez), `cm find "changesets order by changesetid desc limit 200" --format=… --dateformat=yyyy-MM-ddTHH:mm:sszzz --nototal`. Zaman aşımı 30 sn (find sunucuya gider).
...
- **Sunum:** `PlasticSourceControlView` — Plastic SCM rozeti, branch (`/main/release`) + `cs:N`, `repo@server`; Changes | History anahtarı. Changes satırları dosyayı açar (diff Git'e özgü, sunulmaz); History satırı yorum + `cs:N · owner · süre` + branch rozeti, sağ tık Copy Changeset ID / Copy Comment. Durum sözlüğü Git ile ortaktır (`FileChangeStatus`: CH/CO→modified, AD/CP→added, DE/LD→deleted, MV/LM/RP→renamed, PR→untracked) ki renkler `Theme.fileChangeColor`dan gelsin.
- **Persistence değişmez;** yeni servis `ServiceRegistry.plastic` + `LiveServiceRegistry`'de tek satır, fake'i `FakePlasticService`, store `ShellContext.plastic`.
