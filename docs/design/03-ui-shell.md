...
| `WorkspaceStore` | `openTabs: [String]` (**repo path** — ad-çakışması bug fix'i, karar 11), `activeTab`, sidebar görünürlükleri, repo başına grid layout (`auto/columns/rows` + spec/20'deki matematik girdileri), `isFocusMode`, dialog state'leri, `fileViewerPresentation` | UI intent'leri; açılışta `ui-state.json` (okurken tek seferlik ad→path migration) | Persist edilen her alan mutasyonu → `config.updateUIState` (servis 500ms debounce'lar). Tab kapatma: minimize guard'ı (`CloseTabDialog`) → repo terminallerini kill → persist |
...
- **Unified diff modu (karar 4):** `GitServicing`'in tiplenmiş `UnifiedDiff` modeli; satır-başına paragraf arka plan renkleri (ekleme/silme/hunk-başlığı, Theme token'larından) + gutter satır numaralarıyla tek `NSAttributedString`. Üçüncü parti diff renderer'a gerek yok; side-by-side ileri sürüm adayı.
