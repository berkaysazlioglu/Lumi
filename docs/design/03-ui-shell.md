...
- **Frame otoritesi tek yerdedir — `LumiKit.TerminalGridFit`:** emülatör satır/sütun sayısını `Int(boyut / hücre)` ile aşağı yuvarlar ve ızgarayı frame'in sol üst köşesine sabitler; view tam host bounds'una oturtulursa artan boşluk (bir satır yüksekliğine kadar, 13pt'de ~16px) tamamen **alta ve sağa** düşer ve terminal kart çerçevesine asimetrik oturur. Bu yüzden view, ızgaranın fiilen kapladığı alana (hücre boyutunun tam katı) oturtulup host içinde **ortalanır**; artık iki yana eşit paylaştırılır. Hedef frame hücre boyutundan (`TerminalGridSizing.cellSize`) aritmetikle türetildiği için **idempotenttir** — her layout'ta çağrılır, delta yoksa `false` döner ve gereksiz redraw/`onRedraw` doğurmaz; küçültme satır/sütunu değiştirmediği için ek PTY resize da doğmaz. `TerminalHostContainer`, `attachView` reassert'i ve `refreshAttachedViews` aynı fonksiyonu çağırır; view'lara `autoresizingMask` **verilmez** (ızgara katı olmayan bir boyuta esnetip aradaki karelerde gereksiz cols/rows değişimi doğururdu). Font değişimi hücre boyutunu değiştirdiğinden `TerminalSession.setFont` host'a `needsLayout` işaretler.
