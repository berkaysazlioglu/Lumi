...
**Hareket (`Theme+Motion.swift`).** Süreler `quick 0.12` / `standard 0.2` / `panel 0.3` / `pulse 1` + hazır `Animation` sabitleri (`quickEase`, `standardEase`, `standardOut`, `panelEase`, `statusPulse`). Gecikmeler ayrı `Duration` sabitleridir: `hoverRevealDelay 500ms`, `hoverOpenDelay 350ms`, `hoverCloseDelay 200ms`, `sidebarRevealDelay 100ms`, `searchDebounce 150ms`. StatusDot durum renk sistemi (working=success+pulse, waiting-unseen=warning+pulse, …) birebir korunur. Kart header'ında durum noktasının hemen sağında `TerminalIdentityIcon` durur (karar 45; Orca `TerminalTabLeadingIcon`): `TerminalMeta.provider` Claude/Codex ise bundle logosu (`ProviderIcon`), `nil` (düz shell) ise `terminal` SF Symbol'ü — "ne durumda" ve "kim" iki ayrı gliftir.
...
Karar 47 ile sol varsayılan sıra Sessions → Projects'tir; ilk sürümdeki ters sıra migration ile düzeltilir. Karar 48 ile `ProjectWorkspaceStore.addedProjects`, bağımsız `sidebarProjectPaths` seçimini çözer. Otomatik root keşfi üst repo seçici için korunur. Plastic formu mevcut branch'i varsayılan seçer; yeni branch isteğe bağlıdır.

Karar 49: `ProjectsPanel` üç katmandır — proje satırı, `CheckoutRow` (`.original(Repo)` `primary` rozetiyle / `.workspace(ProjectWorkspace)` branch etiketiyle) ve altında `AgentRow` listesi. Ajan modeli `TerminalListStore.terminals(in:)`'den türer; glif `AgentActivityIcon(AgentActivityState)` ve kimlik `TerminalIdentityIcon` ayrı çizilir, zaman `RelativeTimeFormatter.shortLabel(meta.lastActivityAt)`. Birden fazla ajan `N agents` toggle'ı ile daraltılır (view-lokal state). Sağ tık menüleri native `contextMenu`dur; eylemler `ShellContext.focusAgent` / `requestDeleteWorkspace` / `confirmDeleteWorkspace(force:)` / `cancelDeleteWorkspace` / `forgetWorkspace` koordinasyonlarına ve `ShellActions.revealPath`e gider. `DeleteWorkspaceDialogOverlay` (`OverlayID.deleteWorkspaceDialog`) `ModalOverlay + Panel(.modal)` ile çizilir; store'daki `deleteError` ve `canForceDelete` butonu `Force Delete`e çevirir. Oluşturma satırı `WorkspaceOperationRow`a ayrıldı.

Projects `+` ve topbar `+`, aynı `RepoSelectorView` bileşenini farklı `excludedRepoPaths` ile sunar. Topbar açık tab'ları dışlar; Projects kendi seçili projelerini ve yönetilen çocuk workspace'leri dışlar. Sidebar picker `sidebarProjectSelector` dialoguna bağlıdır ve `ShellContext.addSidebarProject` yalnız seçimi kaydeder, Finder açmaz veya tab açmaz. `RepoFeatureAssembly.configDidChange` ayrı sidebar seçimini store'a yansıtır.
